...
function getClassName(listPrice){
  const {amount}=listPrice
  if (amount > 150) return 'red'
  if (amount < 20) return 'green'    
  }
  
function getTxtPublished(publishedDate){
    const year = new Date()
    return (publishedDate < year.getFullYear() - 10) ? 'Vintage':'New'
}

function getTxtOnSale(listPrice) {
    if (listPrice.isOnSale) return "./BooksImages/icons8-sale-48.png"
}
